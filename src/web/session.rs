...
pub(crate) struct SessionMiddleware<Store> {
    store: Store,
    cookie_path: String,
    cookie_name: String,
...
impl<Store: SessionStore> SessionMiddleware<Store> {
    ...
    /// );
    /// ```
    pub(crate) fn new(store: Store, secret: &[u8]) -> Self {
        Self {
            store,
            save_unchanged: true,
            cookie_path: "/".into(),
            ...
            session_ttl: Some(Duration::seconds(24 * 60 * 60)),
    ...
    pub(crate) fn with_cookie_path(mut self, cookie_path: impl AsRef<str>) -> Self {
    ...
    pub(crate) fn with_session_ttl(mut self, session_ttl: Option<Duration>) -> Self {
    ...
    pub(crate) fn with_cookie_name(mut self, cookie_name: impl AsRef<str>) -> Self {
    ...
    pub(crate) fn without_save_unchanged(mut self) -> Self {
    ...
    pub(crate) fn with_same_site_policy(mut self, policy: SameSite) -> Self {
        self.same_site_policy = policy;
        self
    }

    /// Sets the domain of the cookie.
    pub(crate) fn with_cookie_domain(mut self, cookie_domain: impl AsRef<str>) -> Self {
    ...
    fn verify_signature(&self, cookie_value: &str) -> std::result::Result<String, &'static str> {
...
impl<State> SessionExt for Request<State> {
    fn session(&self) -> &Session {
        self.ext::<Session>()
            .expect("request session not initialized")
    }

    fn session_mut(&mut self) -> &mut Session {
        self.ext_mut().expect("request session not initialized")
    }
}
